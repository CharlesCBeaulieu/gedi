...
class ScanIdentification2:
    ...
    def coarse_filtering(self, scan_path):
        ...
        print(f"{'Index':<6}{'Cad':<10}{'Score(lower is better)':<25}{'Eigenvalues':<30}")
        print("-" * 70)
        for idx, (cad, score) in enumerate(sorted_cad_score.items(), start=1):
            ...
            print(f"{idx:<6}{cad:<10}{score:<10.4f}{eig_values}")

        return sorted_cad_score

    def fine_filtering(self, scan_path, candidate_cads):
        print("-----------------Fine filtering-----------------")
        start = time.time()

        scan_desc = np.load(os.path.join(self.scan_desc_folder, os.path.basename(scan_path).split(".")[0] + ".npy"))
        ...
        for cad in tqdm(candidate_cads, desc="Processing cads for fine filtering"):
            ...
            registration_result[cad]["fitness"] = est_result.fitness
            registration_result[cad]["inlier_rmse"] = est_result.inlier_rmse
            registration_result[cad]["transformation"] = est_result.transformation
